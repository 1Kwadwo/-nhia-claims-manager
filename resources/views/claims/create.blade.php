                    ...
                    <form action="{{ route('claims.store') }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('date_of_service') is-invalid @enderror" 
                                           id="date_of_service" name="date_of_service" value="{{ old('date_of_service') }}" required>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('visit_type') is-invalid @enderror" 
                                        ...
                                        <option value="Maternity" {{ old('visit_type') == 'Maternity' ? 'selected' : '' }}>Maternity</option>
                                        <option value="Referral" {{ old('visit_type') == 'Referral' ? 'selected' : '' }}>Referral</option>
                                    </select>
                                    @error('visit_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('diagnoses') is-invalid @enderror" 
                                      id="diagnoses" name="diagnoses" rows="3" placeholder="Enter diagnoses (one per line)">{{ old('diagnoses') }}</textarea>
                            @error('diagnoses')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
